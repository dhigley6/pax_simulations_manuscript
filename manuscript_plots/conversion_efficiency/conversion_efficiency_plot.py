...
ag_imfp = {'x': np.array([50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500]),
           'y': np.array([5.3, 4.3, 5.3, 6.5, 7.7, 8.9, 10.0, 11.1, 12.1, 13.2, 14.2, 15.2, 16.2, 17.1, 18.1, 19.0])}
au_imfp = {'x': np.array([50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500]),
           'y': np.array([3.6, 3.5, 4.6, 5.7, 6.8, 7.9, 8.9, 9.9, 10.9, 11.8, 12.8, 13.7, 14.6, 15.5, 16.3, 17.2])}
al_imfp = {'x': np.array([54.6, 60.3, 66.7, 73.7, 81.5, 90.0, 99.5, 109.9, 121.5,
                          134.3, 148.4, 164.0, 181.3, 200.3, 221.4, 244.7, 270.4,
                          298.9, 330.3, 365.0, 403.4, 445.9, 492.7, 544.6, 601.8,
                          665.1, 735.1, 812.4, 897.8, 992.3, 1096.6, 1212.0,
                          1339.4, 1480.3, 1636.0, 1808]),
           'y': np.array([0.357, 0.368, 0.381, 0.397, 0.415, 0.435, 0.458, 0.483,
                          0.511, 0.541, 0.575, 0.611, 0.651, 0.693, 0.739, 0.788,
                          0.841, 0.899, 0.966, 1.03, 1.10, 1.18, 1.27, 1.36,
                          1.46, 1.57, 1.69, 1.83, 1.97, 2.13, 2.30, 2.49, 2.69,
                          2.91, 3.15, 3.42])}
pt_imfp = {'x': np.array([54.6, 60.3, 66.7, 73.7, 81.5, 90.0, 99.5, 109.9, 121.5,
                          134.3, 148.4, 164.0, 181.3, 200.3, 221.4, 244.7, 270.4,
                          298.9, 330.3, 365.0, 403.4, 445.9, 492.7, 544.6, 601.8,
                          665.1, 735.1, 812.4, 897.8, 992.3, 1096.6, 1212.0,
                          1339.4, 1480.3, 1636.0, 1808]),
           'y': np.array([0.501, 0.483, 0.467, 0.453, 0.442, 0.436, 0.435,
                          0.437, 0.442, 0.450, 0.460, 0.472, 0.488, 0.506,
                          0.527, 0.550, 0.577, 0.608, 0.642, 0.680, 0.722,
                          0.768, 0.818, 0.872, 0.931, 0.996, 1.07, 1.14,
                          1.22, 1.31, 1.41, 1.52, 1.63, 1.76, 1.89, 2.04])}
...
def conversion_plot():
    ...
    ag_conversion_efficiency = {'x': phot,
                                'y': calculate_conversion_efficiency(phot, ag_3d_cross_section, ag_imfp, ag_3d_5half_binding, ag_atomic_density)}
    phot = np.linspace(200, 1500, 1000)
    au_conversion_efficiency = {'x': phot,
                                'y': calculate_conversion_efficiency(phot, au_4f_cross_section, au_imfp, au_4f_7half_binding, au_atomic_density)}
    al_conversion_efficiency = {'x': phot,
                                'y': calculate_conversion_efficiency(phot, al_2p_cross_section, al_imfp, al_2p_3half_binding, al_atomic_density)}
    pt_conversion_efficiency = {'x': phot,
                                'y': calculate_conversion_efficiency(phot, pt_fermi_cross_section, pt_imfp, pt_fermi_binding, pt_atomic_density)}
    plt.figure(figsize=(3.37, 3.5))
    plt.semilogy(ag_conversion_efficiency['x'], ag_conversion_efficiency['y'], label='Ag 3d')
    plt.semilogy(au_conversion_efficiency['x'], au_conversion_efficiency['y'], label='Au 4f')
    plt.semilogy(al_conversion_efficiency['x'], al_conversion_efficiency['y'], label='Al 2p')
    plt.semilogy(pt_conversion_efficiency['x'], pt_conversion_efficiency['y'], label='Pt Fermi')
...
def calculate_conversion_efficiency(phot, cross_section, imfp, binding_energy, number_density):
    ...
    conversion_efficiency = number_density*(cross_interp*1E-18)*(imfp_interp/1E7)
