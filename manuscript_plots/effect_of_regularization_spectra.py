...
def run_sim():
    ...
    results = {"4": results_4, "7": results_7}
...
def _format_figure(f, axs):
    ...
    plt.tight_layout()
    regularizer_widths = pax_simulation_pipeline.DEFAULT_PARAMETERS[
        "regularizer_widths"
    ]
    ...
    axs[1].text(
        0.27, 0.73, "underfit", ha="center", clip_on=False, transform=axs[0].transAxes
    )
    axs[1].text(
        0.27, 0.43, "good", ha="center", clip_on=False, transform=axs[0].transAxes
    )
    axs[1].text(
        0.27, 0.15, "overfit", ha="center", clip_on=False, transform=axs[0].transAxes
    )
    axs[1].text(
        1.96,
        0.73,
    ...
    axs[1].text(
        1.96, 0.15, "good", ha="center", clip_on=False, transform=axs[0].transAxes
    )
    big_ax = _make_big_dummy_ax(plt.gcf())
    big_ax.plot([], [], "k--", label="Ground Truth")
    big_ax.plot([], [], "r", label="Deconvolved")
    big_ax.legend(
        loc="upper center", frameon=True, borderpad=0.3, framealpha=1, borderaxespad=0.3
    )
    big_ax.set_xticks([])
    big_ax.set_yticks([])
...
def _regularization_offset_plot(ax, data_list):
    for ind, data in enumerate(data_list):
        ...
        ax.plot(
            data.deconvolved_x,
            offset + data.deconvolved_y_ / norm,
            "r",
            label="Deconvolved",
        )
        ax.plot(
            data.deconvolved_x,
            offset + data.ground_truth_y / norm,
            "k--",
            label="Ground Truth",
        )
...
def _make_big_dummy_ax(fig):
    ...
    dummyax.tick_params(
        labelcolor="none", top="off", left="off", right="off", bottom="off"
    )
