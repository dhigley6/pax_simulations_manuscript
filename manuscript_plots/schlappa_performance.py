...
def _load_data(log10_counts_to_load):
    ...
    for i in log10_counts_to_load:
        data_list.append(
            pax_simulation_pipeline.load(i, rixs="schlappa", photoemission="ag")
        )
...
def _format_figure(f, ax_irf, ax_pax, ax_spectra, grid, spectra_log10_counts):
    ...
    texts.append(
        ax_spectra.text(
            7.8,
            2.4,
            "$N_e=10^" + str(int(spectra_log10_counts[2])) + "$",
            ha="center",
            transform=ax_spectra.transData,
        )
    )
    texts.append(
        ax_spectra.text(
            7.8,
            1.4,
            "$N_e=10^" + str(int(spectra_log10_counts[1])) + "$",
            ha="center",
            transform=ax_spectra.transData,
        )
    )
    texts.append(
        ax_spectra.text(
            7.8,
            0.4,
            "$N_e=10^" + str(int(spectra_log10_counts[0])) + "$",
            ha="center",
            transform=ax_spectra.transData,
        )
    )
